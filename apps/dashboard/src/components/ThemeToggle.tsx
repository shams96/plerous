...
export default function ThemeToggle() {
  ...
  return (
    <button
      ...
      style={{
        background:   dark ? 'rgba(155,89,211,0.10)' : 'rgba(92,45,142,0.07)',
        borderColor:  dark ? 'rgba(155,89,211,0.22)' : 'rgba(92,45,142,0.20)',
      }}
    >
      {/* Track */}
      <span className="relative inline-flex items-center" style={{ width: 40, height: 22 }}>
        <span
          className="absolute inset-0 rounded-full transition-all duration-300"
          style={{ background: dark ? '#2D1A42' : '#E8D5F8' }}
        />
        {/* Knob */}
        <span
      ...
      <span
        ...
        style={{ color: dark ? 'rgba(155,89,211,0.80)' : '#5C2D8E' }}
      >
        {dark ? 'Dark' : 'Light'}
      </span>
    </button>
