  ...
  async function handleNPILookup(e: React.FormEvent) {
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        setError('Plerous API server is not reachable. Please make sure it is running.')
        return
      }
      ...
      if (res.status === 503) {
        setError(data.error || 'Plerous API server is not running. Start it and try again.')
        return
      }
